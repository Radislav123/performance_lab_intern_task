...
test_data = [
    (f"task2/test_data/{x}/circle_file.txt", f"task2/test_data/{x}/points_file.txt", f"task2/test_data/{x}/results.txt")
    for x in range(len(os.listdir("task2/test_data")))
]
...
def test(circle_filepath: str, points_filepath: str, results_filepath: str):
    with patch.object(sys, "argv", [".\\task1\\task2.py", circle_filepath, points_filepath]), \
